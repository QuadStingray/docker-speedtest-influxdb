...
case class ResultConverter() {
  ...
  def convertToPoint(measurementResult: SpeedTestResult, clientInfos: ClientInfos): Point = {
    val measurement = Point.measurement("speedtest").time(System.currentTimeMillis, TimeUnit.MILLISECONDS)

    measurement.tag(TagHost, measurementResult.connectionInfo.server)
  ...
  def convertToMap(measurementResult: SpeedTestResult, clientInfos: ClientInfos): Map[String, Any] = {

    val mutableMap = mutable.Map[String, Any]()
    mutableMap.put(TagHost, measurementResult.connectionInfo.server)
